...
def RC(In):
    ...
    if rho_s >= rho_comY:				# 압축철근 항복 (압축철근이 항복하기 위한 최소 철근비)
        ...
        c = (-q + np.sqrt(q**2 - 4*p*r))/(2*p);  a = beta1*c;  comSteel_Status = 'Not Yielding'
    ...
    if ep_t < ep_smin:  Failure_Mode = 'Concrete Crushing'
	# % 강도감소계수 (phi) : 띠철근 기준
    ...
    R.ep_s1 = ep_s1;		R.f_s1 = f_s1;				R.comSteel_Status = comSteel_Status;	R.ep_t = ep_t
...
def FRP(In):
    ...
    for kk in range(3):  # 3번 반복        
        ...
        if rho_f >= rho_fbD:  # Over-reinforced : Concrete crushing
            Failure_Mode = 'Concrete Crushing';  rho_Check = 'OK (Over-reinforced)';
            ...
            Failure_Mode = 'FRP Rupture';  rho_Check = 'OK (Under-reinforced)'
